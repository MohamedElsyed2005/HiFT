...
# 1. np.array.astype(np.float) is deprecated in NumPy >=1.20 and REMOVED in
#    NumPy >=1.24. Changed np.float → float (Python builtin) to avoid
#    AttributeError that would crash every training batch.
# 2. No logic changes — augmentation pipeline is correct.
# ============================================================
...
class Augmentation:
    ...
    def _crop_roi(self, image, bbox, out_sz, padding=(0, 0, 0)):
        ...
        c = -a * bbox[0]
        d = -b * bbox[1]
        # FIX: np.float is removed in NumPy>=1.24; use float (Python builtin)
    ...
    def _shift_scale_aug(self, image, bbox, crop_bbox, size):
        ...
        # Adjust target bbox relative to new crop origin
